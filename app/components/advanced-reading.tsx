...
function ReadingDecision({ unit }: { unit: ReadingUnit }) {
  return <section className="advanced-decision">
    <div><span>MAKE THE CALL · UNIT {unit.number}</span><small>Prepare before live</small></div>
...
function ReadingHeroVisual({ data }: { data: SessionReadingData }) {
  const visualHeader = <header className="reading-visual-header"><span>WEEK {data.number} OUTPUT</span><b>{data.checkpoint}</b></header>;
  ...
  return <section className="reading-hero-visual loop-visual" aria-label="Commit adapt and learn decision loop">
    ...
    <div className="loop-demo"><span>DEMO DAY</span><b>Present how the judgment evolved—not only where it ended.</b></div>
  </section>;
}
...
export default function AdvancedReading({ data }: { data: SessionReadingData }) {
  return <main className="noted-briefing session-page advanced-reading">
    <header className="noted-topbar">
      ...
      <div className="noted-topbar-actions session-topbar-actions"><span>Session {data.number} · Reading · 30–40 min</span><Link href="/noted">Meet Noted ←</Link></div>
    </header>

    <section className="advanced-reading-hero">
      <div className="advanced-reading-hero-copy">
        <p className="noted-kicker">Session {data.number} · {data.arc}</p>
        <h1>{data.title}</h1>
        <p>{data.description}</p>
        <div className="noted-actions"><a className="noted-primary" href="#orientation">Begin Session {data.number} ↓</a><Link className="noted-secondary" href={`/session-${data.number}`}>Session overview</Link></div>
      </div>
      <ReadingHeroVisual data={data} />
    </section>

    <nav className="noted-reading-map" aria-label={`Session ${data.number} contents`}>
      <span>Session map</span><a href="#orientation">00 · Start here</a>{data.units.map((unit) => <a href={`#unit-${unit.number}`} key={unit.number}>{unit.number} · {unit.shortTitle}</a>)}<a href="#checkpoint">Checkpoint</a>
    </nav>

    <section className="noted-section advanced-orientation" id="orientation">
      <div className="noted-section-heading"><span>Start here · The reading contract</span><h2>{data.orientation}</h2><p>Read for the reasoning, not the labels. Each exhibit reinforces the argument that comes before it; the live session will pressure-test the same concepts through the evolving Noted case.</p></div>
    ...
    {data.units.map((unit, unitIndex) => <section className={`session-unit advanced-unit ${unitIndex % 2 ? "alternate" : ""}`} id={`unit-${unit.number}`} key={unit.number}>
      <header className="session-unit-header"><div><span>UNIT {unit.number} · {unit.eyebrow}</span><small>{unit.title}</small></div><b>{unit.number}</b></header>
    ...
    <section className="session-checkpoint advanced-checkpoint" id="checkpoint" aria-label={`Week ${data.number} checkpoint`}>
      <header><span>WEEK {data.number} · CHECKPOINT</span><h2>{data.checkpoint}</h2><p>{data.checkpointIntro}</p></header>
    ...
    <section className="advanced-next">
      <div><span>SESSION {data.number}</span><b>{data.checkpoint}</b><p>Complete this week’s linked decision record.</p></div><i aria-hidden="true" />{data.number < 4 ? <Link className="next advanced-next-link" href={`/session-${data.number + 1}`}><span>{data.next.label}</span><b>{data.next.title}</b><p>{data.next.copy}</p><strong>Open Session {data.number + 1} overview <em aria-hidden="true">›</em></strong></Link> : <div className="next"><span>{data.next.label}</span><b>{data.next.title}</b><p>{data.next.copy}</p></div>}
    </section>
    <footer className="noted-footer"><span>CORE / PM · Session {data.number} reading</span><span>Product context from Noted; staged evidence is constructed for practice unless explicitly labeled real.</span></footer>
  </main>;
}
